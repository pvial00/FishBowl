...
void fishbowlCBCEncrypt52(char *infile_name, long long fsize, char *outfile_name, int key_length, int iv_length, int mac_length, int kdf_iterations, unsigned char * kdf_salt, unsigned char *key, int bufsize) { 
    FILE *infile, *outfile;
    unsigned char *keyprime[52] = {0};
    int Ka[26] = {0};
    int Kb[26] = {0};
    ...
    memset(state.R, 0, 16*(10*(sizeof(int))));
    ...
    convertKeytoNum52(Ka, Kb, keyprime, key_length);
    genRoundKeys(&state, Ka, (key_length / 2));
    genSBoxes(&state, Kb, (key_length / 2));
    subRoundKeys(&state);
...
void fishbowlCBCDecrypt52(char *infile_name, long long fsize, char *outfile_name, int key_length, int iv_length, int mac_length, int kdf_iterations, unsigned char * kdf_salt, unsigned char *key, int bufsize) {
    ...
    outfile = fopen(outfile_name, "wb");
    convertKeytoNum52(Ka, Kb, keyprime, key_length);
    genRoundKeys(&state, Ka, 26);
    genSBoxes(&state, Kb, 26);
